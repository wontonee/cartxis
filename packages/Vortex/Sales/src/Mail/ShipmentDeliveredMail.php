...
class ShipmentDeliveredMail extends Mailable
{
    ...
    {
        $template = EmailTemplate::findByCode('shipment_delivered');
        ...
        return $this->subject('Your order has been delivered - ' . $this->shipment->shipment_number)
            ->view('sales::emails.shipment-delivered')
            ->with([
    ...
    private function getTemplateData(): array
    {
        return [
            ...
            'delivered_at' => $this->shipment->delivered_at?->format('F j, Y, g:i a') ?? now()->format('F j, Y, g:i a'),
