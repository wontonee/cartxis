...
class CartTaxCalculator
{
    ...
    public function calculate(array $cartItems, array $customerAddress = []): array
    {
        ...
        if (function_exists('Hook') && class_exists(\Vortex\Core\Facades\Hook::class)) {
            \Vortex\Core\Facades\Hook::doAction('cart.tax.calculating', $cartItems, $customerAddress);
        }
        ...
        if (function_exists('Hook') && class_exists(\Vortex\Core\Facades\Hook::class)) {
            $result = \Vortex\Core\Facades\Hook::applyFilter('cart.taxes', $result, $cartItems, $customerAddress);
        }

        // Fire hook after calculation
        if (function_exists('Hook') && class_exists(\Vortex\Core\Facades\Hook::class)) {
            \Vortex\Core\Facades\Hook::doAction('cart.tax.calculated', $result, $cartItems);
        }
