...
namespace Vortex\Marketing\Services;

use Vortex\Marketing\Models\Coupon;
use Vortex\Marketing\Models\CouponUsage;
...
class CouponService
{
    ...
    public function validate(string $code, ?int $customerId = null, float $cartTotal = 0, Collection $cartItems = null): array
    {
        ...
        if ($coupon->first_order_only && $customerId) {
            $orderCount = \Vortex\Shop\Models\Order::where('customer_id', $customerId)
        ...
        if ($coupon->customer_groups && $customerId) {
            $customer = \Vortex\Customer\Models\Customer::find($customerId);
