...
const currentImage = computed(() => allImages.value[selectedImage.value] || props.product.image);
                ...
                <div class="space-y-6">
                    ...
                    <div v-if="configurableAttributes && configurableAttributes.length > 0" class="space-y-4 border-t pt-6">
                        <div v-for="attribute in configurableAttributes" :key="attribute.id" class="space-y-2">
                            ...
                            <!-- Color Swatches -->
                            <div v-if="attribute.code === 'color'" class="flex flex-wrap gap-2">
                                <button
                                    ...
                                    :style="option.swatch_value ? { backgroundColor: option.swatch_value } : {}"
                                    :title="option.label"
                                >
                                    <span v-if="!option.swatch_value" class="text-xs font-medium">
                                        {{ option.label.substring(0, 2) }}
                                    </span>
                                    <svg
                                        v-if="selectedAttributes[attribute.code] === option.value"
                                        class="absolute -top-1 -right-1 w-5 h-5 text-indigo-600 bg-white rounded-full"
                                        fill="currentColor"
                                        viewBox="0 0 20 20"
                                    >
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                    ...
                    <div v-if="product.in_stock" class="space-y-4">
                        ...
                        <button
                            ...
                            class="w-full bg-indigo-600 text-white px-8 py-3 rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
                        >
